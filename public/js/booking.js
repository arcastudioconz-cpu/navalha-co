...
document.addEventListener('na:ready', () => {
  ...
  $('#confirmBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      // Real booking revenue now flows into ARCA Intelligence — order_id
      // uses the actual booking ID so a retry/reload never double-counts
      // the same appointment as two separate sales.
      if (window.ARCA) {
        window.ARCA.trackConversion({
          order_id: data.id,
          total: confirmedSvc ? confirmedSvc.price : 0,
          description: confirmedSvc ? confirmedSvc.name : 'Appointment',
